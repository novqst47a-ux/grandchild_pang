...
for (const [density, launcherSize, foregroundSize] of densities) {
  ...
  await Promise.all([
    writeFile(join(directory, 'ic_launcher.png'), icon(launcherSize)),
    writeFile(join(directory, 'ic_launcher_round.png'), icon(launcherSize)),
    writeFile(join(directory, 'ic_launcher_foreground.png'), icon(foregroundSize)),
  ]);
}
