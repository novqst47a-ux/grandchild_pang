...
3. **서비스 워커.** cache-first라 `styles.css`는 물론 `@vite/client`와 의존성 모듈까지 캐시해 개발 중 변경이 반영되지 않았다.
   Phase 1에서 `import.meta.env.PROD` 가드를 넣어 개발 서버에서는 등록하지 않는다. 캐시는 **`v8`**(A4·A5에서 올림).
...
Phase 5 실기기 점검에 **고대비(A5)를 켠 상태**도 추가할 것. Android는
설정 → 접근성 → 고대비 텍스트 / 색상 반전 계열 설정이 `prefers-contrast`로 전달되는지
기기마다 다르므로, 전달되지 않으면 그 사실 자체를 기록해 두면 된다.
